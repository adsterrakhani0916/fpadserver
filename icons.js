...
const ICONS = {
  ...
  badge: `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9.06 11.9 7 15h7l-1.19-3.1"></path><path d="M13.35 11.9 15 4l1.87 7"></path><path d="m5.23 10.64 1.12-7.55 4.26 4.06"></path><path d="m18.77 13.36-1.12 7.55-4.26-4.06"></path><path d="m10.7 13.36 1.12 7.55-4.26-4.06"></path></svg>`,
...
// Create CSS for icons
(function addIconsCSS() {
  const style = document.createElement('style');
  style.textContent = `
    .icon-check {
      background-image: url('data:image/svg+xml;utf8,${encodeURIComponent(ICONS.check)}');
    }
    .icon-clock {
      background-image: url('data:image/svg+xml;utf8,${encodeURIComponent(ICONS.clock)}');
    }
    .icon-zap {
      background-image: url('data:image/svg+xml;utf8,${encodeURIComponent(ICONS.zap)}');
    }
    .icon-badge {
      background-image: url('data:image/svg+xml;utf8,${encodeURIComponent(ICONS.badge)}');
    }
    .icon-arrow-right {
      background-image: url('data:image/svg+xml;utf8,${encodeURIComponent(ICONS.arrowRight)}');
    }
    .icon-send {
      background-image: url('data:image/svg+xml;utf8,${encodeURIComponent(ICONS.send)}');
    }
  `;
